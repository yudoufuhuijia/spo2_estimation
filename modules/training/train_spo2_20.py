"""
SpO2模型训练脚本 - 优化版 v5.0（修复运行错误）
核心修复：
1. 清理逻辑修复：支持删除子目录，避免IsADirectoryError
2. 路径配置修复：移除输出目录字符串中的注释，避免目录名非法
3. 评估路径修复：指向v5.0正确输出目录model_output_v2
"""
import os
import sys
import zipfile
...
class ModelTrainer:
    ...
    def validate_epoch(self, val_loader):
        ...
        return avg_loss, avg_mae
    def train(self, train_loader, val_loader, epochs=20, early_stopping_patience=5, verbose=True, val_every_n_epochs=2):
        """
        v5.0优化版训练方法
        epochs: 训练轮数（默认20）
        val_every_n_epochs: 每N轮验证1次（默认2）
        """
        ...
        self.model.train()
        print(f"\n{'='*70}")
        print(f"开始训练（v5.0: {epochs}轮，每{val_every_n_epochs}轮验证1次）")
        print(f"{'='*70}")
        for epoch in range(epochs):
            # 训练一轮
            train_loss, train_mae = self.train_epoch(train_loader)
            # v5.0: 每N轮验证一次
            if (epoch + 1) % val_every_n_epochs == 0 or epoch == epochs - 1:
                val_loss, val_mae = self.validate_epoch(val_loader)
                history['train_loss'].append(train_loss)
                history['train_mae'].append(train_mae)
                history['val_loss'].append(val_loss)
                history['val_mae'].append(val_mae)
                history['epochs'].append(epoch + 1)  # v5.0: 记录验证轮次
                # 更新学习率
                ...
                if val_loss < best_val_loss:
                    ...
                    patience_counter = 0
                    # v5.0: 保存到新目录
                    torch.save(self.model.state_dict(), 'model_output_v2/best_model.pth')
                    if verbose:
                ...
                # 不验证的轮次也打印训练信息
                if verbose:
                    print(f"Epoch [{epoch+1:3d}/{epochs}] Train Loss: {train_loss:.4f} | Train MAE: {train_mae:.4f}%")
        print(f"\n{'='*70}")
        print(f"训练完成！最佳验证损失: {best_val_loss:.4f}")
        print(f"{'='*70}")
        # v5.0: 生成训练趋势图
        print(f"\n📊 生成训练趋势图...")
        self._generate_training_trend_plot(history)
        return history
    def _generate_training_trend_plot(self, history):
        """v5.0: 生成训练趋势图（和您的图一样的风格）"""
        import matplotlib
        ...
        ax1.set_title('Training/Validation Loss Trend', fontsize=12)
        ax1.legend(fontsize=10)
        ax1.grid(True, alpha=0.3)
        # 右图：MAE趋势
        ax2.plot(epochs, history['train_mae'], 'b-', label='Train MAE', linewidth=2)
        ax2.plot(epochs, history['val_mae'], 'r--', label='Val MAE', linewidth=2)
        ax2.set_xlabel('Epochs', fontsize=11)
        ax2.set_ylabel('MAE (%)', fontsize=11)
        ax2.set_title('Training/Validation MAE Trend', fontsize=12)
        ax2.legend(fontsize=10)
        ax2.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('model_output_v2/training_trend.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"✅ 趋势图已保存: model_output_v2/training_trend.png")
...
class ModelEvaluator:
    ...
    def __init__(self, model_path: str):
        ...
        try:
            ...
            self.model.load_state_dict(checkpoint)  # 修复：原代码多了['model_state_dict']
            self.model.eval()
...
def main():
    print("="*70)
    print("🎯 SpO2模型训练系统 v5.0_最终稳定版（修复运行错误）")
    print("="*70)
    # ==================== OSS配置 ====================
    print("\n【步骤1/7】加载OSS配置...")
    OSS_BUCKET = None
    ...
    # 清理旧数据（修复：支持删除子目录）
    print(f"\n🧹 清理旧数据...")
    for dir_path in ['training_data', 'model_output', 'model_output_v2']:
        if os.path.exists(dir_path):
            shutil.rmtree(dir_path)  # 递归删除目录及所有内容
            print(f"  ✅ 已删除旧目录: {dir_path}")
        os.makedirs(dir_path, exist_ok=True)  # 重建空目录
    print(f"\n🚀 开始处理（{'测试模式' if TEST_MODE else '全量模式'} v5.0）...\n")
    ...
    pipeline = FixedTrainingPipeline(
        ...
        output_dir="model_output_v2",  # 修复：移除注释，避免目录名非法
        device='cpu'
    )
    # 测试模式参数（适配小样本）
    if TEST_MODE:
        epochs = 20  # v5.0: 改为20轮
        batch_size = 2  # 更小的批次
    ...
    # 模型评估（修复：指向v5.0正确输出目录）
    print(f"\n{'='*70}")
    print(f"【步骤7/7】模型评估")
    print(f"{'='*70}")
    predictions_path = "model_output_v2/predictions.npz"  # 修复：路径改为model_output_v2
    model_path = "model_output_v2/best_model.pth"  # 修复：路径改为model_output_v2
    if os.path.exists(predictions_path) and os.path.exists(model_path):
        try:
            ...
            evaluator.plot_results(predictions, labels, save_dir="model_output_v2")  # 修复：保存到v2目录
            # 输出最终结果
            print(f"\n{'='*70}")
            print(f"🎉 训练完成！{'测试模式' if TEST_MODE else '全量模式'} v5.0 运行成功！")
            print(f"{'='*70}")
            # 测试模式结果判断
            if TEST_MODE:
                print(f"\n🧪 测试模式结果判断：")
